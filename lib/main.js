...
//  if(tab.location.href == "about:blank") return;
  ...
  downloadImage(prefs.get("path")+time+"_favicon.ico", tab.favicon);
  
  //url
  writeTextFile(prefs.get("path")+time+".txt", tab.location.href);

  //page
  var document = tab.contentDocument.wrappedJSObject.body;
  writeTextFile(prefs.get("path")+time+"_page.htm", document.innerHTML);
//  writeTextFile(prefs.get("path")+time+"_page.txt", deleteScriptElement(document).textContent);
  writeTextFile(prefs.get("path")+time+"_page.text", document.textContent);
}
